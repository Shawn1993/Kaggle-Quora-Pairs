...
def get_train_features(kf, train_features, train_label):
    ...
    for i, (train_index, dev_index) in enumerate(kf):
        ...
        res = pd.DataFrame(index=dev_index, data={'is_duplicate': bst.predict(train_features.iloc[dev_index])})
        res.index.name = 'id'
        train_feature.append(res)
...
if __name__ == '__main__':
    ...
    train_output.rename(columns={config.COLUMN_LABEL:'lgb_dart'}, inplace=True)
    train_save_path = os.path.join(config.OUTPUT_TRAIN_DIR, 'lgb_dart_%d_%f_%f.csv' % (best_iter, mean, std))
    ...
    test_output.rename(columns={config.COLUMN_LABEL:'lgb_dart'}, inplace=True)
    test_save_path = os.path.join(config.OUTPUT_TEST_DIR, 'lgb_dart_%d_%f_%f.csv' % (best_iter, mean, std))
